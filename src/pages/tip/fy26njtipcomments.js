...
import response from "./FY26NJTIPgetCommentsData.json";
import "./defaults.css";
...
const title = "FFY2026 TIP for New Jersey Public Comments and Responses";
  ...
  const onChange = (e) => {
    ...
    filteredComments = filteredComments.filter(
      (item) =>
        ...
        item.issue_json.toLowerCase().includes(filter)
    );
...
export async function getServerData() {
  try {
    const res = await fetch(
      "https://apps.dvrpc.org/ords/njtip2026comment/publicComments/commentIssueResponse"
    );

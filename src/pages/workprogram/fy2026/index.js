...
const WorkProgramPage = ({
  ...
  serverData: { projects, amendments },
  location,
}) => {
        ...
        {activeFilter === "Work Program Amendments" ? (
          <ul className="list-group">
            {amendments.map((amendment) => (
              <li key={amendment.file_url} className="list-group-item">
                <a target="_blank" href={amendment.file_url}>
...
export async function getServerData() {
  try {
    ...
    return {
      props: { projects: json.items, amendments: json2.items },
    };
  } catch (error) {
    return {
      status: 500,
